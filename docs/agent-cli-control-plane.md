...
| Skill discovery for trigger evals | project `.claude/skills`; primary evidence is `Skill` tool use, path evidence fallback | `$CODEX_HOME/skills` exposed as a skills-only extra read root; primary evidence is a session-rollout `<skill>` injection, completed path evidence fallback | native Agent Skills exist, but autonomous trigger is unadvertised until headless `activate_skill` consent is live-proven | project `.agents/skills`; primary evidence is native `skill` tool call, path evidence fallback |
