...
| Skill discovery | `.claude/skills` plus `Skill` evidence | `$CODEX_HOME/skills` plus session-rollout evidence, path fallback | Native Agent Skills, but `activate_skill` consent blocks a trigger claim until live-proven | `.agents/skills` / `.vibe/skills` plus `skill` evidence | Existence of a skill format is not evidence that autonomous headless activation works. |
