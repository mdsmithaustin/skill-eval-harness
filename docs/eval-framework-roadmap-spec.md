...
(`skill_benchmark.py:11381`) and the fan-out in `prepared_task_rows` (`:1604`).
...
`without_skill` on the same case (`build_paired_summary` (`:15600`)). That number is believable only
...
- **Abstractions used or changed:** none. A test-time guard around `grade_case_variant` (`:14131`).
...
and negative-delta flags (`build_benchmark_report` (`:17163`)) are computed right — sits a layer
...
- **Abstractions used or changed:** `tool_call` and `structured_output` are deterministic, so
  ...
  `assertion_result`. `tool_call` reuses `command_events` (`:6491`) and the `command_order`
  logic; `structured_output` extends `json_field_equals` with JSON-Schema validation.
  `factuality` adds no core code: it is a named rubric that `judge_prompt` (`:11981`) renders
...
- **Abstractions used or changed:** read an optional `judge` block in the manifest; add a check
  in `audit_manifest_report` (`:19986`) comparing the declared judge model against `jetty.model`
...
- **Follow-on:** surface the guide's rules where they are checkable. Extend the messaging in
  `prompt_assertion_leakage_findings` (`:832`) and `fixture_recommendations` (`:19679`) to point
...
- **Abstractions used or changed:** an optional `oracle` tier on an assertion
  ...
  `script` is `demo` unless marked, judge/live are `live`). `build_benchmark_report` (`:17163`)
  reports, per case, the share of its pass rate carried by `strong` oracles;
  `audit_manifest_report` (`:19986`) warns when a case passes only on weak ones. This extends
...
- **Abstractions used or changed:** reads the cross-run history from 2.6. A `prune` report (or a
  flag in `build_benchmark_report` (`:17163`)) marks a case a removal candidate when, across the
  last N runs, it never failed and never showed lift (`with_skill` == `without_skill` every time).
  `audit_manifest_report` (`:19986`) lists the candidates; removal stays a human decision.
...
- **Abstractions used or changed:** `prepared_task_rows` (`:1604`) gains a `model` dimension
  ...
  so grading needs no change. `build_benchmark_report` (`:17163`) groups `by_variant` within
  `by_model`, and `build_paired_summary` (`:15600`) computes lift per (case, model).
...
- **Abstractions used or changed:**
  ...
  - **Anchored `graded_dimensions`** as a `judge` assertion shape:
    ...
    (`:11981`) renders the dimensions; the result carries per-dimension scores in `evidence`.
  ...
  - `grade_case_variant` (`:14131`) splits totals into critical, gated, and soft. A `critical`
  ...
  - **Statistical lift** in `build_paired_summary` (`:15600`): alongside the raw delta, compute a
...
- **Abstractions used or changed:** this lives in the runner, not core grading. Recording sits
  beside `write_trace_artifacts` (`:8392`): a `tool-replay.json` keyed per tool, with
...
- **Abstractions used or changed:** `normalize_trace_record` (`:7449`) and
  `normalize_trace_records` (`:8108`) keep their inputs but emit OTel GenAI semantic-key
...
- **Abstractions used or changed:** a consumer of `build_benchmark_report`. Add an append-only
  ...
  the `compare_results` (`:18345`) logic.
...
- **Abstractions used or changed:** extend `render_viewer` (`:19142`) with a `serve` mode and
...
- **Abstractions changed (core contract):** a case gains an optional `turns` list. The
  ...
  `read_output_base` (`:6338`) and `discover_run_bases` learn the turn layout; runners drive the
...
- **Slice-lift concentration (from the macro-evals notebook):** compute, per slice, where lift (or
  ...
  up from per-case lift. `build_slice_summary` (`:15826`) already groups by domain/difficulty/
