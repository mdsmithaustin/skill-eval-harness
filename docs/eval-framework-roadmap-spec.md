...
(`skill_benchmark.py:11374`) and the fan-out in `prepared_task_rows` (`:1597`).
...
`without_skill` on the same case (`build_paired_summary` (`:15593`)). That number is believable only
...
- **Abstractions used or changed:** none in the engine. Adds
  ...
  `assertion_result` (`:11374`) directly. That function has no direct unit test today, and its
...
- **Abstractions used or changed:** none. A test-time guard around `grade_case_variant` (`:14124`).
...
and negative-delta flags (`build_benchmark_report` (`:17156`)) are computed right — sits a layer
...
- **Abstractions used or changed:** `tool_call` and `structured_output` are deterministic, so
  ...
  `assertion_result`. `tool_call` reuses `command_events` (`:6484`) and the `command_order`
...
- **Abstractions used or changed:** read an optional `judge` block in the manifest; add a check
  in `audit_manifest_report` (`:19979`) comparing the declared judge model against `jetty.model`
...
- **Follow-on:** surface the guide's rules where they are checkable. Extend the messaging in
  `prompt_assertion_leakage_findings` (`:825`) and `fixture_recommendations` (`:19672`) to point
...
- **Abstractions used or changed:** a new type in `TEXT_ASSERTIONS` (`:243`), implemented in
  `assertion_result` (`:11374`). It reads `output.md` (or a named artifact), applies an optional
...
- **Abstractions used or changed:** an optional `oracle` tier on an assertion
  ...
  `script` is `demo` unless marked, judge/live are `live`). `build_benchmark_report` (`:17156`)
  reports, per case, the share of its pass rate carried by `strong` oracles;
  `audit_manifest_report` (`:19979`) warns when a case passes only on weak ones. This extends
...
- **Abstractions used or changed:** extend `run_script_assertion` (`:11242`) and
  `assertion_result` (`:11374`) to optionally parse a score from the oracle's stdout — a JSON line
...
- **Abstractions used or changed:** reads the cross-run history from 2.6. A `prune` report (or a
  flag in `build_benchmark_report` (`:17156`)) marks a case a removal candidate when, across the
  last N runs, it never failed and never showed lift (`with_skill` == `without_skill` every time).
  `audit_manifest_report` (`:19979`) lists the candidates; removal stays a human decision.
...
- **Abstractions used or changed:** `prepared_task_rows` (`:1597`) gains a `model` dimension
  ...
  so grading needs no change. `build_benchmark_report` (`:17156`) groups `by_variant` within
  `by_model`, and `build_paired_summary` (`:15593`) computes lift per (case, model).
...
- **Abstractions used or changed:**
  - `assertion_result` (`:11374`) gains an optional `score` (0-1 or a normalized 1-5) and
  ...
  - `grade_case_variant` (`:14124`) splits totals into critical, gated, and soft. A `critical`
  ...
  - **Statistical lift** in `build_paired_summary` (`:15593`): alongside the raw delta, compute a
...
- **Abstractions used or changed:** this lives in the runner, not core grading. Recording sits
  beside `write_trace_artifacts` (`:8385`): a `tool-replay.json` keyed per tool, with
...
- **Abstractions used or changed:** `normalize_trace_record` (`:7442`) and
  `normalize_trace_records` (`:8101`) keep their inputs but emit OTel GenAI semantic-key
...
- **Abstractions used or changed:** a consumer of `build_benchmark_report`. Add an append-only
  ...
  the `compare_results` (`:18338`) logic.
...
- **Abstractions used or changed:** extend `render_viewer` (`:19135`) with a `serve` mode and
...
- **Abstractions changed (core contract):** a case gains an optional `turns` list. The
  ...
  `read_output_base` (`:6331`) and `discover_run_bases` learn the turn layout; runners drive the
...
- **Slice-lift concentration (from the macro-evals notebook):** compute, per slice, where lift (or
  ...
  up from per-case lift. `build_slice_summary` (`:15819`) already groups by domain/difficulty/
