...
class SkillRootKeyTests(unittest.TestCase):
    ...
    @staticmethod
    def row(path: Path, variant: str, **kwargs) -> dict:
        rows = sb.prepared_task_rows(path, sb.validate_manifest(path), **kwargs)
    ...
    def test_per_skill_manifest_mounts_under_frontmatter_name(self):
        ...
        with tempfile.TemporaryDirectory() as td, tempfile.TemporaryDirectory() as wd:
            ...
            self.assertEqual(build.skill_paths, ["skills/good-pr/SKILL.md"])
            self.assertTrue((Path(wd) / "skills" / "good-pr" / "SKILL.md").is_file())
            self.assertFalse((Path(wd) / "skills" / "SKILL.md").exists())

    def test_repo_root_skill_without_frontmatter_name_is_rejected(self):
        with tempfile.TemporaryDirectory() as td:
            path = make_eval_repo(Path(td), skill_paths=["SKILL.md"])
            repo_root = sb.repo_root_for_manifest(path)
            (repo_root / "SKILL.md").write_text("---\ndescription: d. Use it.\n---\n\n# A\n", encoding="utf-8")
            with self.assertRaisesRegex(sb.AblationError, "name"):
    ...
    def test_nested_skill_md_path_mounts_under_its_directory(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            repo_root = sb.repo_root_for_manifest(path)
            self.assertEqual(sb.skill_root_key(repo_root, "skills/good-pr/SKILL.md"), "good-pr")
            self.assertEqual(self.row(path, "with_skill")["skill_root_keys"], ["good-pr"])
            tree = sb.build_canonical_skill_tree(repo_root, sb.validate_manifest(path), Path(td) / "tree")
    ...
    def test_directory_path_mounts_under_its_own_name(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            manifest["skill_paths"] = ["skills/good-pr"]
            path.write_text(json.dumps(manifest), encoding="utf-8")
            repo_root = sb.repo_root_for_manifest(path)
            self.assertEqual(sb.skill_root_key(repo_root, "skills/good-pr"), "good-pr")
            self.assertEqual(self.row(path, "with_skill")["skill_root_keys"], ["good-pr"])
            tree = sb.build_canonical_skill_tree(repo_root, sb.validate_manifest(path), Path(td) / "tree")
    ...
    def test_two_roots_with_the_same_directory_name_are_rejected(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn("'b/good-pr/SKILL.md'", str(ctx.exception))
            with self.assertRaises(sb.AblationError):
                sb.build_canonical_skill_tree(repo_root, manifest, Path(td) / "tree")
            with self.assertRaises(sb.AblationError):
    ...
    def test_per_skill_layout_ablation_parent_hash_equals_canonical(self):
        ...
        with tempfile.TemporaryDirectory() as td, tempfile.TemporaryDirectory() as wd:
            ...
            path = make_eval_repo(root, skill_name="good-pr", skill_paths=["SKILL.md"],
                                  skill_text=SKILL_FIXTURE, ablations=[self.SECTION_ABLATION])
            manifest = sb.validate_manifest(path)
            repo_root = sb.repo_root_for_manifest(path)
            res = sb.materialize_ablation(repo_root, manifest, self.SECTION_ABLATION, root / "abl")
            self.assertEqual(res["parent_skill_hash"], sb.canonical_skill_tree_hash(repo_root, manifest))
            self.assertNotEqual(res["skill_hash"], res["parent_skill_hash"])
            self.assertEqual({p.name for p in Path(res["dir"]).iterdir()}, {"good-pr"})
            rows = sb.prepared_task_rows(path, manifest, include_ablations=True, ablation_dir=root / "ablations")
            arow = next(r for r in rows if r["variant"] == "ablation:no-rp")
            wrow = next(r for r in rows if r["variant"] == "with_skill")
            self.assertEqual((wrow["skill_root_keys"], arow["skill_root_keys"]), (["good-pr"], ["good-pr"]))
            self.assertEqual(wrow["skill_tree_hash"], arow["ablation"]["parent_skill_hash"])
            build = sb.build_skill_workspace(sb.PreparedTask.from_row(arow), Path(wd))
            self.assertEqual(build.attestation.mounted_skill_tree_hash, arow["ablation"]["skill_hash"])
            self.assertTrue((Path(wd) / "skills" / "good-pr" / "SKILL.md").is_file())
